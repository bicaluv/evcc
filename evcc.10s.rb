...
#
#  <xbar.var>string(VAR_SERVER="http://raspberrypi:7070"): You local evcc instance.</xbar.var>
...
def buildMode(mode, modeText, lp)
    curMode = lp["mode"]
    lpId = lp["loadpoint"] - 1
    url = "#{ENV['VAR_SERVER']}/api/loadpoints/#{lpId}/mode/#{mode}"
    selfCall = "| shell=\"#{File.expand_path(__FILE__)}\" param1=writeApi param2=#{url} terminal=false refresh=true"

    "--" << ((curMode == mode) ? CHECKMARK << " " : "") << modeText << selfCall
end
...
activeLoadpoints.each do |lp|
    ...
    puts buildMode("off", "Stop", lp)
    puts buildMode("now", "Sofort", lp)
    puts buildMode("minpv", "Min + PV", lp)
    puts buildMode("pv", "PV", lp)
